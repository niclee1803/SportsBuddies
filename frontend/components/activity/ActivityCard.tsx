...
const ActivityCard = ({ activity }: ActivityCardProps) => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            setCreatorInfo({
              ...
              profilePicUrl: userData.profilePicUrl || "https://placehold.co/36"
            });
          }
        } catch (error) {
  ...
  const getActivityStatus = () => {
    ...
    switch(activity.status.toLowerCase()) {
      case "pending":
        return { label: "Pending", color: "#FF9500", icon: "hourglass" };
      case "full":
        return { label: "Full", color: "#5856D6", icon: "people" };
      default:
        // Default to upcoming for active/created status
        return { label: "Upcoming", color: "#34C759", icon: "time" };
    }
  };
